  ...
  const saveChanges = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/workout-templates/${id}`, {
        ...
        body: JSON.stringify({ title, isGlobal, exercises }),
      });
      if (res.ok) {
        navigate(-1);
      } else {
      ...
      console.error("Ошибка при PATCH шаблона", err);
    } finally {
      setSaving(false);
    }
  };
                ...
                <Accordion.Panel>
                  <Stack>
                    {items.map((name) => {
                      ...
                      return (
                        <Box key={name} style={{ border: "1px solid #ccc", borderRadius: 8, padding: 8 }}>
